...
static volatile bool usb_tx_in_progress = false;
...
{
    int tx_timeout = 500; // 500ms is large enough. if we reach the timeout then com has been lost
    while(usb_tx_in_progress && tx_timeout--)
    {
        osDelay(1);
    }
    ...
    usb_tx_in_progress = true;
    return app_usbd_cdc_acm_write(&m_app_cdc_acm, tx_buffer, size);
}
