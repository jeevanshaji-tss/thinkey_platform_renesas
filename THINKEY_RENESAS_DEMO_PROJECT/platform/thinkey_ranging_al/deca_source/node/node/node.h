...
struct rx_pckt_s
{
    ...
    uint8_t     timeStamp[TS_40B_SIZE];         /* TimeStamp of current RX: blinkRx, pollRx, finalRx */
    uint32_t    rtcTimeStamp;                   /* MCU current Rx RTC timestamp: make sense for the Poll message reception */

    uint8_t     temperature;                    /* raw reading of temperature: valid at reception of Final */
    ...
    uint8_t     acc[(FULL_ACC_LEN-ACC_OFFSET)*4 +1]; /* TODO: review accumulator reading / do we need it in this app ? /
                                                         * Will be started with offset ACC_OFFSET :
                                                         * for ACC_OFFSET = 0
...
struct rx_mail_s
{
    ...
    uint8_t     acc[(FULL_ACC_LEN-ACC_OFFSET)*4 +1];    /* Will be started with offset ACC_OFFSET :
                                                         * for ACC_OFFSET = 0
                                                         * 2x ((1016-0)*4 + 1)= 2x 4065 = 8130 bytes for raw ACC
                                                         *
                                                         * for ACC_OFFSET = 300
...
struct node_info_s
{
    ...
    result_t    result[MAX_KNOWN_TAG_LIST_SIZE]; /* range/X/Y/Xtal_offset result back to the tag */
    ...
    uint8_t      Tmeas;            //copy of Temperature calibration value for DW chip
