...
usb_data_e waitForData(uint8_t *pBuf , uint16_t len, uint16_t *read_offset,uint16_t cyclic_size);
...
usb_data_e waitForCommand(uint8_t *pBuf , uint16_t len, uint16_t *read_offset,uint16_t cyclic_size)
{
    ...
    for (cnt=0;cnt<len;cnt++)//Loop over the buffer rx data
    {
		...
		else
		{
			...
			{//Need to find out if getting regular command or JSON
...
usb_data_e waitForData(uint8_t *pBuf, uint16_t len, uint16_t *read_offset ,uint16_t cyclic_size)
{
    ...
    if((len + dataLen) < sizeof(app.local_buff)-1)
    {
        for (cnt=0;cnt<len && ret!=DATA_READY;cnt++)
        {
            if (!frame_started)
            {
                if (pBuf[*read_offset] == '\n' || pBuf[*read_offset] == '\r')
                {
                    // Do not store CR or LF when not receiving USB2SPI frames. E.g Decaranging
    ...
    { /* overflow in usb2spi protocol : flush the buffer */
        dataLen = 0;
    }
    ...
    if (uartLen > 0)
    {
        if (app.pConfig->s.uartEn)
        {
            ret = (app.mode == mUSB2SPI)?
                waitForData(app.uartRx.buf, uartLen,&tailUart,sizeof(app.uartRx.buf) - 1) :
                waitForCommand(app.uartRx.buf, uartLen,&tailUart,sizeof(app.uartRx.buf) - 1);
        ...
        else
        {
            ...
            app.uartRx.tail = (app.uartRx.tail + uartLen) & (sizeof(app.uartRx.buf) - 1);
        }
    }
    if (usbLen > 0)
    {
        ...
        {
            ret = (app.mode == mUSB2SPI)?
                waitForData(app.usbRx.buf, usbLen,&tailUsb,sizeof(app.usbRx.buf) - 1) :
                waitForCommand(app.usbRx.buf, usbLen,&tailUsb,sizeof(app.usbRx.buf) - 1);
