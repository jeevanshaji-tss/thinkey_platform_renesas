...
#define S_UPDATE_UWB_PDOAOFF 0xFFFF0100
#define S_UPDATE_UWB_POWER 0xFFFF0001
/**
 *
...
#if (defined(CUSTOM_BOARD_INC) && (CUSTOM_BOARD_INC == type2ab_evb)) /*added on June 30,2021*/
    pbss->s.update_uwb_config_flag = pbss->s.update_uwb_config_flag | S_UPDATE_UWB_PDOAOFF;
#endif /* end of (CUSTOM_BOARD_INC && CUSTOM_BOARD_INC == type2ab_evb) */
    return (CMD_FN_RET_OK);
}
REG_FN(f_rng_offset)
...
REG_FN(f_user_button)
{
    ...
    while (1)
    {
        if(nrf_gpio_pin_read(BUTTON_1) == GPIO_PIN_RESET)
        {
...
REG_FN(f_lstat)
{
    ...
    if(str)
    {
        ...
        sprintf(&str[strlen(str)],"\"SFDD\":%d}}",(int)app.event_counts_sfd_detect);
        ...
        dwt_configeventcounters(1); // we need and can clear and enable counters in the chip
...
REG_FN(f_decaJuniper)
{
    ...
    if(str)
    {
        ...
        sprintf(&str[strlen(str)],"\"Apps\":[\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\"],\r\n",
		NODE_FUNC, TAG_FUNC, TRILAT_FUNC, USPI_FUNC, TCWM_FUNC, TCFM_FUNC, LISTENER_FUNC);
...
REG_FN(f_jstat)
{
    ...
    if(str)
    {
        ...
        sprintf(&str[strlen(str)],"\"RCDEL\":%d}}",         (unsigned int)pbss->s.rcDelay_us);
        ...
        sprintf(&str[strlen(str)],"\"TXPOWER\":0x%x,\r\n",    (int)pbss->s.txConfig.power);
        sprintf(&str[strlen(str)],"\"PGDLY\":0x%x,\r\n",    (int)pbss->s.txConfig.PGdly);
        sprintf(&str[strlen(str)],"\"PGCOUNT\":0x%x,\r\n",    (int)pbss->s.txConfig.PGcount);
#endif /* end of (CUSTOM_BOARD_INC && CUSTOM_BOARD_INC == type2ab_evb) */
...
REG_FN(f_power)
{
    ...
    if(str)
    {
...
#if (defined(CUSTOM_BOARD_INC) && (CUSTOM_BOARD_INC == type2ab_evb)) /*added on June 30,2021*/
            pbss->s.update_uwb_config_flag = pbss->s.update_uwb_config_flag | S_UPDATE_UWB_POWER;
#endif /* end of (CUSTOM_BOARD_INC && CUSTOM_BOARD_INC == type2ab_evb) */
        }
        else if(n != 1)
        {
        ...
        sprintf(&str[strlen(str)],"\"PGCOUNT\":\"0x%08X\"}}",(unsigned int)pbss->s.txConfig.PGcount);
...
REG_FN(f_stskeyiv)
{
    ...
    if(str)
    {
        ...
        sprintf(&str[strlen(str)],"\"STS_STATIC\":\"%d\"}}",(int)pbss->s.stsStatic);
...
REG_FN(f_xtal_trim)
{
    ...
    if(str)
    {
        ...
        else
        {
            ...
            xtalTrim = dwt_getxtaltrim() | (app.pConfig->s.xtalTrim & ~XTAL_TRIM_BIT_MASK);
        }

        app.pConfig->s.xtalTrim = xtalTrim; // it can have the 0x80 bit set to be able overwrite OTP values during APP starts.
...
REG_FN(f_help_std)
{
    ...
    if(str)
    {
        ...
        while (known_commands[indx].cmnt != NULL)
        {
            ...
            if((mode & mMASK) == app.mode || (mode & mMASK) == mANY || mIDLE == app.mode )
            {
...
REG_FN(f_save)
{
    ...
    case mTRILAT_N :
        ...
        break;
    default:
        app.pConfig->s.default_event = 0; //default app is defined in the main();
        break;
    }
...
const char STD_CMD_COMMENT         []={"This command described in the documentation"};

const char COMMENT_RCDEL           []={"Time, \'us\', between end of Tag's Blink_TX and Tag start Rx of Ranging Config message. From Node's view this is a delay between end of reception of Blink's data and start of transmission of preamble of the Ranging Config message. This time should be set to the same value on the Tag and on the Node."};
const char COMMENT_REPDEL          []={"Time, \'us\', when Tag shall be ready to receive the Node's Response after end of transmission of the Poll (wait4response). This parameter is defined by the Node and sent to the Tag in the Ranging Config message."};
const char COMMENT_P2FDEL          []={"Time, \'us\',  when Tag shall transmit Final's RMARKER, calculated from Tag's Poll's RMARKER. This parameter is defined by the Node and sent to the Tag in the Ranging Config message."};
...
const char COMMENT_HELP            []={"This command displays the help information. Usage: \"help\" or \"help <CMD>\", <CMD> is the command from the list, i.e. \"help tag\"."};

const char COMMENT_NODE            []={"Slotted DS-TWR PDoA Node application."};
const char COMMENT_TAG             []={"Slotted DS-TWR Tag application."};
const char COMMENT_TRILAT          []={"Trilateration application runs on top of the Node application. It requires at least 3 Tags with configured and placed to X-Y-Z corners of the room to demonstarte the Trilateration Location Solver."};
const char COMMENT_USPI            []={"Starts the USB2SPI mode. This is a special binary protocol to read/write registers of DW chip using external application. DecaRanging application prior to 3.24 require this mode to be started manually."};
...
const char COMMENT_TCFM            []={"Test Continuous Frame mode is to transmit packets for test purposes. Usage: \"tcfm <NUM> <PAUSE> <LEN>\" <NUM>>: number of packets to transmit. <PAUSE>: pause in between packets in ms. <LEN>: length of the transmit payload, bytes\r\n"};
...
const char COMMENT_X           []={"X-coordinate of this Tag in the room, mm. Used only in the tag, which is a part of a Trilateration example running on the fift unit."};
const char COMMENT_Y           []={"Y-coordinate of this Tag in the room, mm. Used only in the tag, which is a part of a Trilateration example running on the fift unit."};
const char COMMENT_Z           []={"Z-coordinate of this Tag in the room, mm. Used only in the tag, which is a part of a Trilateration example running on the fift unit."};

const char COMMENT_ADDR            []={"Sets the 16-bit address to the Node.\r\nUsage: \" \""};
const char COMMENT_PANID           []={"Sets the 16-bit PanID for the Node.Node will use this PANID to setup the Tag. \r\nUsage: \" \""};
const char COMMENT_NUMSLOT         []={"Sets the number of Slots used for Ranging. \r\nUsage: \"numslot 20\". Cannot be smaller than MAX_KNOWN_TAG_LIST_SIZE"};
const char COMMENT_SLOTPER         []={"Sets the slot period in ms.\r\nUsage: \"slotper 5\""};
const char COMMENT_SFPER           []={"Sets the superframe period in ms.\r\nUsage: \"sfper 100\". Cannot be smaller than NUM_SLOTS*SLOT_PERIOD"};

const char COMMENT_JSTAT           []={"Status in JSON format"};
const char COMMENT_GETDLIST        []={"Discovered Tags's <UI64_HEX> list in JSON format."};
const char COMMENT_GETKLIST        []={"Known Tags list in JSON format."};
const char COMMENT_ADDTAG          []={"Add the tag <UI64_HEX> to Known list using specified parameters.\r\nUsage: \"addtag <UI64_HEX> <NEW_UI16_HEX> <SLOW> <FAST> <MODE>\""};
const char COMMENT_DELTAG          []={"Delete tag <UI16/64_HEX> from Known Tags list\r\nUsage: \"deltag <UI16/64_HEX>\""};
...
const char COMMENT_UART            []={"Switch the interface between UART and USB CDC.\r\rUsage: \"uart <ENABLE_UART>\"\r\n ENABLE_UART: 1 to use UART, 0 to use USB CDC"};
...
const char COMMENT_D2K             []={"Service command to add all Tag from the DList to the KList"};
