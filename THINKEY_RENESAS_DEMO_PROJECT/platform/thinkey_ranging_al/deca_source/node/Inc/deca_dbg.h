...
#define DBG_TRACE_EVENTS    if(! TX_EVENT_RECEIVED ) printf("MsgPtrs [ %d - %d %d %d - %3d - 0x%X] \n",\
                                                        rxData[phyEvent[app.eventIdx].rxDataPtr].rxDataLen,    \
                                                        phyCfg.eventIdx, app.eventIdx, phyEvent[app.eventIdx].type,    \
                                                        rxData[phyEvent[app.eventIdx].rxDataPtr].msg.stdMsg.seqNum,    \
                                                        rxData[phyEvent[app.eventIdx].rxDataPtr].msg.stdMsg.messageData[0]);

#define DBG_TRACE_CLOCK_SYNC_TX  printf("CS Sent at .. @ .. 0x%02x%02x%02x%02x%02x \n",    phyEvent[app.eventIdx].timeStamp[4],\
...
#define DBG_TRACE_CLOCK_SYNC_RX  printf("CS Received .. @ .. 0x%02x%02x%02x%02x%02x \n",    phyEvent[app.eventIdx].timeStamp[4],\
                                                                                phyEvent[app.eventIdx].timeStamp[3],\
                                                                                phyEvent[app.eventIdx].timeStamp[2],\
                                                                                phyEvent[app.eventIdx].timeStamp[1],\
                                                                                phyEvent[app.eventIdx].timeStamp[0] );\
...
#define DBG_TRACE_TAG_BLINK         printf("Tag Blink : Seq : %d : Time : %lld \n", rxMsg.msg.blinkMsg.seqNum, phyEvent[app.eventIdx].timeStamp);\
                                 //printf("S:%d\n", rxMsg.msg.blinkMsg.seqNum);

#define DBG_TRACE_NETWORK_MSG      printf("Received Network Configuration : %s %d \n", (app.rtls_info.master?"MASTER":"SLAVE"),app.rtls_info.id);

#define DBG_TRACE_CONFIGURATION printf("Configuration Message %x %x : Time : %lld \n", \
                                                            rxData[phyEvent[app.eventIdx].rxDataPtr].msg.stdMsg.messageData[0], \
                                                            rxData[phyEvent[app.eventIdx].rxDataPtr].msg.stdMsg.messageData[1], \
...
#define diag_printf(...) do{\
                            THINKEY_DEBUG_INFO(__VA_ARGS__);\
                         }while(0)
...
#define DBG_TRACE(x)    {uint8_t buf;buf = x;dwt_writetodevice(0xb,0,1,&buf);}
