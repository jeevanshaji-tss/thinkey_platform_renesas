 ...
 * @file      app.h
 *
 * @brief     Decawave Application Layer header contains all macros and structures related apllicaitions
 *
 * @author    Decawave
 *
...
/* ENABLE_USB_PRINT Macro is commented then Segger RTT Print will be enabled*/
#define ENABLE_USB_PRINT

 /* SPI/USART/USB buffers */
#define UART_RX_BUF_SIZE       0x800 /*0x100*/    /**< Read buffer for UART reception, shall be 1<<X */
...
struct app_s
{
    ...
    volatile mode_e mode;           /**< Information: handle the current "mode" of operation */
    ...
    //TDoA anchor branch
    task_signal_t    ctrlServer[NUM_BH_BUFFERS]; /* Set of Ctrl Servers 0 is local;
                                                    [1..NUM_BH_BUFFERS-1] are UWB_BH proxies */    
    ...
    //PDoA branch
    osMailQId        (rxPcktPool_q_id);    /**< Mail queue ID for PDoA Node processes: FreeRTOS does not free the resources */
    ...
    void                (*timer2Isr)(void);//*< DSR callback to execute on Slow APP Timer
    osTimerId           appTimerHandle;
