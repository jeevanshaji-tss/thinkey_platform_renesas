...
/*
 * @brief   configure tan/node application: frame filtering, PANID, address, antenna delays
 *
 *
 * @note
 * */
...
/**
 * channel 5 : DWT_DGC_SEL_CH5
 * channel 9 : DWT_DGC_SEL_CH9
 **/
uint8_t chan_to_index(uint8_t i)
{
    uint8_t ret = DWT_DGC_SEL_CH5;
...
void dw3xxx_calibration_parameters_update(dwt_calibrate_t * dwt_calibrate)
{
    ...
    dwt_calibrate[DWT_DGC_SEL_CH5].dwt_txconfig.PGdly = (uint8_t)(buf >> 24);
    dwt_calibrate[DWT_DGC_SEL_CH9].dwt_txconfig.PGdly = (uint8_t)(buf >> 16);

    /** PG_CNT */
    buf = 0;
    dwt_otpread(PG_CNT_ADDRESS, &buf, 1);
    dwt_calibrate[DWT_DGC_SEL_CH5].dwt_txconfig.PGcount = (uint16_t)(buf >> 16);
    dwt_calibrate[DWT_DGC_SEL_CH9].dwt_txconfig.PGcount = (uint16_t)buf;

    /** TX Power*/
    buf = 0;
    dwt_otpread(TX_CH5_ADDRESS, &buf, 1);
    dwt_calibrate[DWT_DGC_SEL_CH5].dwt_txconfig.power = buf;

    buf = 0;
    dwt_otpread(TX_CH9_ADDRESS, &buf, 1);
    dwt_calibrate[DWT_DGC_SEL_CH9].dwt_txconfig.power = buf;

    /** Antenna Delay */
    buf = 0;
    dwt_otpread(ANT_DLY_ADDRESS, &buf, 1);
    dwt_calibrate[DWT_DGC_SEL_CH5].internal_ant_delay = (uint16_t)(buf >> 16);
    dwt_calibrate[DWT_DGC_SEL_CH9].internal_ant_delay = (uint16_t)buf;

    /** PDoA Code*/
    buf = 0;
    dwt_otpread(PDOA_CODE_PRF64, &buf, 1);
    dwt_calibrate[DWT_DGC_SEL_CH5].pdoacode = (uint16_t)(buf >> 16);

    buf = 0;
    dwt_otpread(PDOA_CODE_PRF64, &buf, 1);
    dwt_calibrate[DWT_DGC_SEL_CH9].pdoacode = (uint16_t)buf;
}
