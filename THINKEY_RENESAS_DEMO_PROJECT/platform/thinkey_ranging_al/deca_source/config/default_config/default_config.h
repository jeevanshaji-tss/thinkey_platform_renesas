...
#define DEFAULT_UART                0      /**< Use UART I/O instead of USB CDC */
#if (defined(CUSTOM_BOARD_INC) && (CUSTOM_BOARD_INC == type2ab_evb)) /*added on March 05,2021*/
#define DEFAULT_ANTD                16 /*Total antenna delay*/
...
/* Slot configuration */
#define DEFAULT_SLOT_PERIOD_MS      5       /**< Slot period (ms), the best of what the implementation can achieve */
#define DEFAULT_NUM_SLOTS           20      /**< Number of slots: cannot be shorter than MAX_KNOWN_TAG_LIST_SIZE */
 ...
 * Node uses these parameters to setup its receiver during TWR exchange with a Tag.
 * Tag hardware should be capable of performing the timings below:
 * */
#define DEFAULT_TAG_POLL_TX_RESPONSE_RX_US  (700)   /**< time when Tag shall be ready to receive the node's Response after end of transmission of the Poll.
...
#if (defined(CUSTOM_BOARD_INC) && (CUSTOM_BOARD_INC == type2ab_evb)) /*added on March 05,2021*/
#define DEFAULT_TAG_POLL_TX_FINAL_TX_US     (2500)  /**< time when Tag shall transmit Final's RMARKER, calculated from Tag's Poll's RMARKER. */
#else /* else of (CUSTOM_BOARD_INC && CUSTOM_BOARD_INC == type2ab_evb) */
#define DEFAULT_TAG_POLL_TX_FINAL_TX_US     (2000)  /**< time when Tag shall transmit Final's RMARKER, calculated from Tag's Poll's RMARKER. */
#endif /* end of (CUSTOM_BOARD_INC && CUSTOM_BOARD_INC == type2ab_evb) */
...
struct sfConfig_s
{
    ...
    uint16_t    tag_replyDly_us;         /**< wait4response delay after end of tag's Poll transmission in us */
    uint16_t    tag_pollTxFinalTx_us;    /**< PollTxToFinalTx period, us (i.e. period from Poll's RMARKER to Final's RMARKER)*/
};
...
struct run_s
{
    ...
    uint8_t     uartEn;         /**< Use UART to output data simultaneously to USB */
    ...
    uint8_t     faultyRanges;   /**< Tag config: number of faulty ranges after that Tag return back to a Discovery phase */
    ...
    int16_t    antRx_a;        /**< antenna delay values for the left port */
    int16_t    antTx_a;        /**< antenna delay values for the left port */
    ...
    int         default_event;  /**< this even will be generated after the Reset: default is set in main(), also see the "save" command */

    uint32_t    lotId;          //workaround for ES0 chip variant:: ES0 has no lotId
    uint32_t    partId;         //workaround for ES0 chip variant:: ES0 has no partId

    dwt_sts_cp_key_t    stsKey;         /**< AES Key to be used to set the STS */
    dwt_sts_cp_iv_t     stsIv;          /**< AES IV to be used to set the initial IV */
    int                 stsStatic; /**< Configuration to STS/IV : value 0 = dynamic STS, 1 = fixed STS*/
    uint8_t             xtalTrim;
    uint32_t            update_uwb_config_flag; /*murata*/
...
struct param_block_s
{
    ...
    uint8_t         free[FCONFIG_SIZE -
                         ...
                         - sizeof(ver_num_t) - sizeof(static_ip_t) - sizeof(run_t)];
}__attribute__((__packed__));
