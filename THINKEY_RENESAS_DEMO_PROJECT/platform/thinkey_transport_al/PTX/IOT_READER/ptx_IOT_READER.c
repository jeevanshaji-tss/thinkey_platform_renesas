...
ptxStatus_t ptxIoTRd_InitNSC(ptxIoTRd_t *iotRd, ptxIoTRd_ComInterface_Params_t *initParams)
{
	...
	plat_initializer.baudRate = 0;
	plat_initializer.flowControl = 0;

	if(NULL != initParams)
	{
		plat_initializer.baudRate = initParams->Speed;
		plat_initializer.flowControl = initParams->FlowControl;
...
ptxStatus_t ptxIoTRd_Init(ptxIoTRd_t *iotRd, ptxIoTRd_InitPars_t *initParams)
{
    ...
    if (iotRd != NULL)
    {
		...
		{
			plat_initializer.baudRate = initParams->ComInterface->Speed;
			plat_initializer.flowControl = initParams->ComInterface->FlowControl;
		}
        ...
        if (ptxStatus_Success == st)
        {
            ...
            if (ptxStatus_Success == st)
            {
                ...
                (void)ptxNSC_GetInitConfigParams(iotRd->Nsc, plat_initializer.baudRate, plat_initializer.flowControl, &con_Uart_Config[0]);
            ...
            if (ptxStatus_Success == st)
            {
                ...
                /* NSC_RFCONFIG_CMD for PTX100R. Use default settings. */
    ...
    if (PTX_COMP_CHECK(iotRd, ptxStatus_Comp_IoTReader))
    {
        ...
        if (ptxStatus_Success == st)
        {
            ...
            if (NULL != discConfig)
            {
                ...
                {
                    st = PTX_STATUS(ptxStatus_Comp_POS, ptxStatus_NotPermitted);
                }
                ...
                if (ptxStatus_Success == st)
                {
                    ...
                    if (1 == discConfig->PollTypeA)
                    {
                        rf_disc_params.Con_Poll            |= (uint8_t)(Init_Poll_A << 4u);
                    }
                    else if (1 == discConfig->PollTypeB)
                    {
                        rf_disc_params.Con_Poll            |= (uint8_t)(Init_Poll_B << 4u);
                    }
                    else if ((1 == discConfig->PollTypeF212) || (1 == discConfig->PollTypeF424))
                    {
                        rf_disc_params.Con_Poll            |= (uint8_t)(Init_Poll_F << 4u);
                    }
                    else if (1 == discConfig->PollTypeV)
                    {
...
ptxStatus_t ptxIoTRd_Update_ChipConfig (ptxIoTRd_t *iotRd, uint8_t nrConfigs, ptxIoTRd_ChipConfig_t *configParams)
{
    ...
    if (PTX_COMP_CHECK(iotRd, ptxStatus_Comp_IoTReader) && (NULL != configParams) && (0 != nrConfigs))
    {
        ...
        {
            if ((NULL != &configParams[i]) && ((RF_LAST_RF_CONFIG_ENTRY > configParams[i].ID)))
            {
                ptxNSC_RfConfig_ParamList_t nsc_id = ptxIoTRd_MapRFConfigID(configParams[i].ID);

                if((NULL != configParams[i].Value) && (0 != configParams[i].Len) && (nsc_id != RfCfgParam_Undefined))
                {
                    nsc_rf_cfg_params[rfconfig_tlv_count].ID = nsc_id;
                    nsc_rf_cfg_params[rfconfig_tlv_count].Value = configParams[i].Value;
                    nsc_rf_cfg_params[rfconfig_tlv_count].Len = configParams[i].Len;
                    rfconfig_tlv_count++;
                    update_rf_cfg = 1u;
                }
            } else if((NULL != &configParams[i]) && (0 != update_sys_cfg))
            {
                switch (configParams[i].ID)
                {
                    case SYS_ThermalThreshold:
                        sys_cfg_params.PowerAmpTempThreshold = configParams[i].Value[0];
                        break;

                    case SYS_OvercurrentThreshold:
                        sys_cfg_params.PowerAmpOverCurrThreshold = configParams[i].Value[0];
                        break;

                    case SYS_ConClockSrc:
                        sys_cfg_params.ConClkSource = configParams[i].Value[0];
                        break;

                    case SYS_ConVarLBS:
                        sys_cfg_params.ConVarLBS = configParams[i].Value[0];
                        break;

                    case SYS_ConNHost:
                        (void)memcpy(&sys_cfg_params.conHost[0], &configParams[i].Value[0], 0x04u);
                        break;

                    case SYS_ConNHosCE:
                        (void)memcpy(&sys_cfg_params.conHostCE[0], &configParams[i].Value[0], 0x04u);
                        break;
        ...
        if ( Prot_T3T == iotRd->CardRegistry->ActiveCardProtType)
        {
            ...
            if (ptxStatus_Success == st)
            {
               ...
               // (void) ptxIoTRd_ClearRfMsgRcvd(iotRd, &iotRd->RfMsg);
                ...
                /* T3T-response data is build up as follows
                 * Offset 0...................: Total Length
                 * Offset 1 - Total Length - 2: Payload
                 * Offset Total Length - 1....: Status Byte (Bit 7 indicates RF-Error)
                 *
                 * Resulting buffer content will be build up as follows
                 * - Lengh of Payload incl. length-byte itself of card response "n"
                 * - Payload of card response "n"
                 * - Lengh of Payload incl. length-byte itself of card response "n + 1" (if present)
                 * - Payload of card response "n + 1" (if present)
                 * - ...
                 * */
                ...
                do
                {
                    if (num_bytes_pending_to_read > max_num_bytes_to_read)
                    {
                        ...
                        {
                        	if (0 == (trx_buffer[max_num_bytes_to_read_temp - 1] & 0x80))
                        	{
                                rx[num_bytes_read] = (uint8_t)max_num_bytes_to_read_temp;
                                num_bytes_read++;
                                (void)memcpy(&rx[num_bytes_read], &trx_buffer[0], max_num_bytes_to_read_temp - 1);
                        	}
                        }
                        num_bytes_pending_to_read -= max_num_bytes_to_read_temp;
                        num_bytes_read += max_num_bytes_to_read_temp - 1;
                    } else
                    {
                        ...
                        {
                        	if (0 == (trx_buffer[num_bytes_pending_to_read - 1] & 0x80))
                        	{
                                rx[num_bytes_read] = (uint8_t)num_bytes_pending_to_read;
                                num_bytes_read++;
                                (void)memcpy(&rx[num_bytes_read], &trx_buffer[0], num_bytes_pending_to_read - 1);
                        	}
                        }
                        num_bytes_read += num_bytes_pending_to_read - 1;
                        num_bytes_pending_to_read = 0;
                    }
...
ptxStatus_t ptxIoTRd_TempSensor_Calibration (ptxIoTRd_t *iotRd, uint8_t Tambient, uint8_t *Tshutdown)
{
    ...
    if ((NULL != iotRd) && (NULL != Tshutdown))
    {
       ...
       // THINKEY_DEBUG_INFO("\n Value of sens_value %d:", sens_val);

       // THINKEY_DEBUG_INFO("\n Value of status1 %d:", status);

        if(ptxStatus_Success == status)
        {
           // THINKEY_DEBUG_INFO("\n Value of status2 %d:", ptxStatus_Success);
            /*
           ...
           // THINKEY_DEBUG_INFO(0,"\n Value of status3 %d:", ptxStatus_Success);
           // THINKEY_DEBUG_INFO("\n Value of temp_offset %d:", temp_offset);
           // THINKEY_DEBUG_INFO("\n Value after 1st call of temp_val %d:", temp_val);
            if(ptxStatus_Success == status)
            {
                /*
                ...
                if(sens_val >= temp_val)
                {
                   ...
                   // THINKEY_DEBUG_INFO("\n Value of temp_offset1 %d:", temp_offset);
                } else
                {
                    temp_offset = (int8_t)(temp_val - sens_val);
                  //  THINKEY_DEBUG_INFO("\n Value of temp_offset2 %d:", temp_offset);
                    temp_offset = (int8_t)(0 - temp_offset);
                    //THINKEY_DEBUG_INFO("\n Value of temp_offset3 %d:", temp_offset);
                }
                /*
                 * Calculate compensated shutdown temperature.
                 */
                temp_val = *Tshutdown;
               // THINKEY_DEBUG_INFO("\n Value of temp_val %d:", temp_val);
               // THINKEY_DEBUG_INFO("\n Value of status4 %d:", status);
                status = ptxIoTRd_TempOffsetComp (iotRd, &temp_offset, &temp_val);
               // THINKEY_DEBUG_INFO("\n Value of temp_offset4 %d:", temp_offset);
               // THINKEY_DEBUG_INFO("\n Value of status5 %d:", status);
            }
...
static ptxStatus_t ptxIoTRd_TempOffsetComp (ptxIoTRd_t *iotRd, int8_t *tempOffset, uint8_t *tempVal)
{
    ...
    if ((NULL != iotRd) && (NULL != tempOffset) && (NULL != tempVal))
    {
       ...
       // THINKEY_DEBUG_INFO("\n Value of temp_offset5 %d:", temp_offset);
        uint32_t rest = 0;
        uint32_t temp_calculated = *tempVal;
       // THINKEY_DEBUG_INFO("\n Value of temp calculated1 %d:", temp_calculated);
        temp_calculated = (49u * temp_calculated + 11060u) * 2550u;
       // THINKEY_DEBUG_INFO("\n Value of temp calculated2 %d:", temp_calculated);
        rest = temp_calculated % 180000u;

        temp_calculated = temp_calculated / 180000u;
       // THINKEY_DEBUG_INFO("\n Value of temp calculated3 %d:", temp_calculated);
        if(rest >= 90000u)
        {
            /* Round up */
            temp_calculated += 1;
           // THINKEY_DEBUG_INFO("\n Value of temp calculated4 %d:", temp_calculated);
        }
        ...
        if(temp_offset >= 0)
        {
           ...
           // THINKEY_DEBUG_INFO("\n Value of temp_offset6 %d:", temp_offset);
            //THINKEY_DEBUG_INFO("\n Value of temp calculated5 %d:", temp_calculated);

        } else
        {
            //Offset is negative. Make it positive and subtract.
            temp_calculated -= (uint8_t)(((int8_t)0 - temp_offset) & 0x007F);
            //THINKEY_DEBUG_INFO("\n Value of temp_offset7 %d:", temp_offset);
           // THINKEY_DEBUG_INFO("\n Value of temp calculated6 %d:", temp_calculated);
        }
       // THINKEY_DEBUG_INFO(0,"\n Value of temp calculated7 %d:", temp_calculated);
        if(temp_calculated > 255u)
        {
           // THINKEY_DEBUG_INFO("\n Value of status6 %d:", st);
            st = PTX_STATUS(ptxStatus_Comp_IoTReader, ptxStatus_InvalidParameter);
           // THINKEY_DEBUG_INFO("\n Value of status7 %d:", st);
        } else
        {
            *tempVal = (uint8_t)temp_calculated;
        }
    } else
    {  // THINKEY_DEBUG_INFO(0,"\n Value of st before %d:", st);
        st = PTX_STATUS(ptxStatus_Comp_IoTReader, ptxStatus_InvalidParameter);
    //THINKEY_DEBUG_INFO("\n Value of st after %d:", st);
}
...
static ptxStatus_t ptxIoTRd_GetRfDiscParams(ptxIoTRd_t *iotRd, ptxNSC_RfDiscPars_t *discoverParams)
{
    ...
    if ((iotRd != NULL) && (discoverParams != NULL))
    {
        ...
        /* NFC-Forum Polling Mode. First technology to poll: NFC-A */
...
#ifndef PTX30W_LISTENER
            /* Reserve the next available 'free' event record. */
            // TODO --> Why this issue is happening ?
            ...
            if ( ptxStatus_Success == st )
            {
                ...
                switch (event->EventId)
                {
                    ...
                    case NSC_EventRfDeact:
                        if (RF_DISCOVER_STATUS_NO_CARD == pIot->DiscoverState)
                        {
