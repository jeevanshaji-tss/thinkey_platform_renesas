 ...
 * @{
 */
#define PTX_T3T_MIN_TX_BUFFER_SIZE   (uint32_t)256              /**< Minimum TX Buffer Size */                    
#define PTX_T3T_NFCID2_SIZE          (uint8_t)8                 /**< NFCID2 Size */
/** @} */
...
/**
 * \brief Initialize / Open the T3T Native Tag Component.
 *
 * \param[in]   t3tComp             Pointer to an existing instance of the T3T component.
 ...
 * \param[in]       NoS                 Number of Services from which to read blocks.
 * \param[in]       serviceCodeList     Contains the service codes from which to read blocks.
 * \param[in]       serviceCodeListLen  Length of the service code list.
 * \param[in]       NoB                 Number of blocks to be read.
 * \param[in]       blockList           List of blocms to be read in the services.
 * \param[in]       blockListLen        Length of the block list.
 * \param[out]      rx                  Pointer to Rx-buffer where the received data from the card shall be stored.
 * \param[in,out]   rxLen               Size of Rx-buffer (in), Length of the received data (out).
 * \param[in]       msTimeout           Timeout in ms that the function is going to wait for receiving data from the card.
 *
 ...
 * \param[in]       MRTICheck           SENSF_RES value for timeout calculation.
 * \param[in]       MRTIUpdate          SENSF_RES value for timeout calculation.
 * \return Status, indicating whether the operation was successful. See \ref ptxStatus_t.
 */
...
/**
 * \brief Unitialize / Close the T3T Native Tag Component.
 *
 * \param[in]   t3tComp             Pointer to an existing instance of the T3T component.
 *
 * \return Status, indicating whether the operation was successful. See \ref ptxStatus_t.
